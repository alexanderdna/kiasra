...
{
	return Env->locals[index].getRaw();
}
...
#define CHECK_GET_INDEX(val) if (index > obj.getLength() - 1) \
	{ \
		KniThrowException(hKEnv, Env->exceptions.indexOutOfRange); \
		return val; \
	}
